...
// lat - latitude
// long - longitude
void drawSphere(GLfloat radius, int latitude, int longitude, int cuts,int stacks)
{
  //Equ. for a Sphere
  ...
  int numCut = longitude/cuts;
  int numSlice = latitude/stacks;
  ...
  for(j=0;j<=longitude;j = numCut + j)
    {
      for(i=0;i<=latitude;i = numSlice + i)
	{
...
// lat - latitude
// long - longitude
void drawIris(GLfloat radius, int latitude, int longitude, int cuts,int stacks)
{
  //Equ. for a Sphere
  //x = x_0 + r*cos(phi)*sin(theta) 
  //y = y_0 + r*sin(phi)*sin(theta) 0 <= phi <= 2*pi , 0 <= theta <= pi )
  ...
  for(j=0;j<=longitude;j = numCut + j)
    {
      for(i=0;i<=latitude;i = numSlice + i)
	{
	  if (i < 5) glColor3f(black.r, black.g, black.b);
	  else glColor3f(seaGreen.r, seaGreen.g, seaGreen.b);
...
void drawEye(GLfloat radius, int blink) {
  glRotatef(90.0, 0.0, 1.0, 0.0);

  glColor3f(azure.r,azure.g,azure.b);
  glTranslatef(0.0,0.0,0.0);
  drawSphere(radius, 360, 180, 45, 45);  // Eye
  glRotatef(-90.0, 0.0, 1.0, 0.0);
  drawIris(radius+0.007, 15, 360, 13, 13);  // Iris
  glRotatef(90.0, 0.0, 1.0, 0.0);

  // blink is between 0.0 - Eye Open and 90.0 - Eye Closed
   glColor3f(green.r, green.g, green.b);
   drawSphere(radius + 0.03, 180, 90+blink, 45, 45); // Upper Lid


  glRotatef(-90.0-blink, 0.0, 0.0, 1.0);
  glColor3f(blue.r, blue.g, blue.b);
  drawSphere(radius + 0.03, 180, 90+blink, 45, 45); // Lower Lid
