...
Particle pp[N]; //Array to hole particles
GLuint  texture[0];         // Storage For Our Particle texture
float scale = N*3/(log(N));
...
void updatePressure(){
	...
	int d = P/2;
	for(n=0;n<N;n++){
		int x = (int)(pp[n].x*6+d);
		int y = (int)(pp[n].y*6+d);
		int z = (int)(pp[n].z*6+d);
...
void initParticle(int n, float xi, float yi, float zi){
	...
	pp[n].x=(rand()%100/100.0f-0.5f)*0.5f+xi;
	pp[n].z=(rand()%100/100.0f-0.5f)*0.5f+zi;
	pp[n].y=(rand()%100/100.0f-0.5f)*0.3f+yi;
	pp[n].t = rand()%100/100+0.5f;
	pp[n].dx = rand()%40 - 20;
	pp[n].dy = rand()%100;
	pp[n].dz = rand()%40 - 20;
}
...
void updateParticle(int n){
	...
	pp[n].x+=pp[n].dx/scale;
	pp[n].y+=(pp[n].dy)/scale - g;
	pp[n].z+=pp[n].dz/scale;	
	float d = (float)P/2;	
	int x = (int)(pp[n].x+d);
	int y = (int)(pp[n].y+d);
	int z = (int)(pp[n].z+d);
	if(x<P-1 && x>0 && y<P-1 && y>0 && z<P-1 && z>0 && pp[n].t>0.2f){
		float dv = rho[x][y][z];
		...
		float xi = 5.0f*cosf(t/100.0f);
		float zi = 5.0f*sinf(t/90.0f);;
		initParticle(n, xi, 0.0f, zi);
	}
