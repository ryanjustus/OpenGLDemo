...
void drawCone(float radius, float radius2, float height, rgb color1, rgb color2){
	//
	//glPushMatrix();
	//glPolygonMode( GL_FRONT , GL_FILL);
	//glPolygonMode( GL_BACK , GL_FILL);
	...
	glBegin(GL_TRIANGLE_STRIP);{ 
	  ...
	  for(i=0;i<=sides;i++){
		...
		float x = cos(theta);
		glNormal3f(x,0,z);	
		glVertex3f(radius2*x,0,radius2*z);		
		glVertex3f(radius*x,height,radius*z);			
		theta+=dTheta;
	  }
	}glEnd();	
	...
	glBegin(GL_TRIANGLE_FAN);{
		...
		int i=0;
		for(i=0;i<=sides;i++){		
			glVertex3f(radius2*cosf(theta),y,radius2*sinf(theta));
			theta+=dTheta;			
		}		
	}glEnd();
	//glPopMatrix();
